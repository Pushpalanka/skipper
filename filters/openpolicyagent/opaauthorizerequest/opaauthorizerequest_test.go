...
func TestAuthorizeRequestFilterOnConfigs(t *testing.T) {
...
func TestAuthorizeRequestFilterOnRequestURL(t *testing.T) {
	for _, ti := range []struct {
		msg               string
		filterName        string
		extraeskipBefore  string
	...
	}{
		{
			...
			filterName:        "opaAuthorizeRequest",
			bundleName:        "somebundle.tar.gz",
			regoQuery:         "envoy/authz/allow_simple_path",
			requestPath:       "/allow",
			requestMethod:     "GET",
			contextExtensions: "",
			expectedStatus:    http.StatusOK,
		...
		},
		{
			...
			filterName:        "opaAuthorizeRequest",
			bundleName:        "somebundle.tar.gz",
			regoQuery:         "envoy/authz/allow_path_space",
		...
		},
		{
			...
			filterName:        "opaAuthorizeRequest",
			extraeskipBefore:  `setPath("/new_set_path") ->`,
			bundleName:        "somebundle.tar.gz",
			regoQuery:         "envoy/authz/allow_set_path",
			...
			opaControlPlane := opasdktest.MustNewServer(
				opasdktest.MockBundle("/bundles/"+ti.bundleName, map[string]string{
					"main.rego": `
						...
						allow_set_path {
							input.attributes.request.http.path == "/new_set_path"
						}
...
func TestAuthorizeRequestFilterOnHeaders(t *testing.T) {
	for _, ti := range []struct {
		msg               string
		filterName        string
		extraeskipBefore  string
...
func TestAuthorizeRequestFilterOnBody(t *testing.T) {
	for _, ti := range []struct {
		msg               string
		filterName        string
		extraeskipBefore  string
